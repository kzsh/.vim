...
if !exists("g:ath_require_js_config_file")
...
fun! RJS_LoadRequireList()
    try
        ...
        let files = RJS_Trim(split(matchstr(define, 'define\s*(\s*\[\zs\_.\{-}\ze\]'), ','))
        let vars = RJS_Trim(split(matchstr(define, 'function\s*(\zs\_.\{-}\ze)'), ','))
...
"===============================================================================
" from: https://github.com/alarie/requirejs.vim
"===============================================================================
"===============================================================================
...
fun! RJS_OpenFile(file)
    ...
    if !empty(file)
        ...
        if match(file, '^dojo/text!') == -1
            call s:RJS_OpenJSFile(file)
        else
            let file = substitute(file, '^dojo/text!', '', '')
